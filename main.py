...
ui_texts = {
    ...
    "Hindi": {
        ...
        "sms_success": "📱 स्थानीय लोगों को अलర్ట संदेश भेजे गए हैं।",
...
for key in ["advice", "map_data", "generated_time", "geo_info", "sms_success", "sms_time"]:
...
if analyze:
    ...
    if not geo:
        ...
        st.stop()
    lat, lon, addr = geo
    st.session_state.geo_info = f"📍 {addr} ({lat:.4f}, {lon:.4f})"
    ...
    # 📲 SMS Alert Dispatch
    local_contacts = ["+916301475493"]
    alert_msg = f"🚨 {severity.upper()} ALERT for {place}!\n{advice_translated}"
    if send_alert_sms(local_contacts, alert_msg):
        st.session_state.sms_success = True
        st.session_state.sms_time = datetime.now().strftime("%I:%M %p • %b %d, %Y")

# ✅ Persistent display for geolocation + SMS success
if st.session_state.geo_info:
    components.html(f"""
    <div style="background:#e8f2ff;padding:0.8rem 1rem;border-radius:12px;margin:1rem 0;
                animation:fadeIn 1.2s ease;">
        <strong>{st.session_state.geo_info}</strong>
    </div>
    <style>@keyframes fadeIn{{from{{opacity:0;transform:translateY(10px);}}to{{opacity:1;transform:translateY(0);}}}}</style>
    """, height=60)

if st.session_state.sms_success:
    st.success(f"{ui['sms_success']} (⏱ {st.session_state.sms_time})")

# Persistent display for advice + map
